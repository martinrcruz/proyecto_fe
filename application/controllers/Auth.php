...
class Auth extends CI_Controller
{
	...
	public function get_user_data()
	{
		...
		if ($this->ion_auth->logged_in()) {
			...
			if ($rta = $this->ion_auth->getUserProfile($res->id_usuario)) {
				foreach ($rta->result() as $r) {
					$row = new stdClass();
					$row->foto_perfil = $r->FOTO_PERFIL;
					$row->biografia = $r->BIOGRAFIA;
					$row->cargo = $r->CARGO;
					$row->tipo_usuario = $r->TIPO_USUARIO;
					$row->nro_contacto = $r->NRO_CONTACTO;
					$row->residencia = $r->RESIDENCIA;
					$row->nacionalidad = $r->NACIONALIDAD;
					$row->rut = $r->RUT;
					$row->fecha_nacimiento = $r->FECHA_NACIMIENTO;
					$row->fecha_ingreso = $r->FECHA_INGRESO;
				}
				if ($row != null) {
					array_push($res->perfil, $row);
				}
			}
