...
class Serie extends CI_Controller
{
    ...
    public function getSerie()
    {
        if (true) {
            ...
            if ($query = $this->serie_model->getSerie()) {
                foreach ($query->result() as $res) {
                    $row = null;
                    $row = new stdClass();
                    $row->id_serie = $res->id_serie;
                    $row->nombre_serie = $res->nombre;
    ...
    public function getDetalleCotizacionTabla()
    {
        if (true) {
            ...
            if (sizeof($response->errores) == 0) {
                if ($query = $this->serie_model->getDetalleCotizacion($where)) {
                    foreach ($query->result() as $res) {
                        $row = null;
                        $row = new stdClass();
                        $row->id_serie = $res->ID_DETALLE_COTIZACION;
    ...
    public function getDetalleCotizacionById()
    {
        if (true) {
            ...
            if (is_numeric($this->input->post('id_serie'))) {
                $request->id = trim($this->security->xss_clean($this->input->post('id_serie', true)));
            } else { //SI NO, ALMACENAMOS EL ERROR EN UN ARRAY PARA DEVOLVERLO COMO RESPUESTA.
                $response->errores[] = "Ocurrió un problema al obtener la solicitud";
            }

            $request->id ? $where = " AND dc.id_serie=$request->id" : $where = '';
    ...
    public function insertDetalleCotizacion()
    {
        if (true) {
            ...
            if ($query = $this->serie_model->insertDetalleCotizacion('serie', $datos)) {
                $response->proceso = 1;
                $response->id = $query;
                $response->data = $datos;
            } else {
    ...
    public function updateDetalleCotizacion()
    {
        if (true) {
            ...
            if (sizeof($response->errores) == 0) {
                if ($query = $this->serie_model->updateDetalleCotizacion('serie', 'ID_DETALLE_COTIZACION', $datos, $request->id)) {
    ...
    public function deleteDetalleCotizacion()
    {
        if (true) {
            ...
            if ($this->input->post('id_serie')) {
                $request->id = $this->security->xss_clean($this->input->post('id_serie'));
            } else { //SI NO, ALMACENAMOS EL ERROR EN UN ARRAY PARA DEVOLVERLO COMO RESPUESTA.
                $response->errores[] = "Ocurrió un problema al obtener la solicitud";
            }

            $where = " AND id_serie=$request->id";


            //SI ES QUE NO HAY ERRORES, PROCEDEMOS A HACER LA PETICION MEDIANTE UN LLAMADO A LA FUNCION DEL MODELO.
            if (sizeof($response->errores) == 0) {
                if ($this->serie_model->updateDetalleCotizacion("serie", "ID_DETALLE_COTIZACION", array('FECHA_BAJA' => $fecha, "ESTADO" => 0), $request->id)) {
                    //SI EL PROCESO ES EXITOSO, DEVOLVERA UN VALOR DENTRO DEL ARRAY DE RESPUESTA IGUAL A 1
                    $response->proceso = 1;
                }
            } else {
    ...
    public function deleteDetallesCotizacion(){
      if (true) {
          ...
          //SI ES QUE NO HAY ERRORES, PROCEDEMOS A HACER LA PETICION MEDIANTE UN LLAMADO A LA FUNCION DEL MODELO.
          if (sizeof($response->errores) == 0) {
              if ($query = $this->serie_model->deleteDetalles($where, $fecha)) {
                  //SI EL PROCESO ES EXITOSO, DEVOLVERA UN VALOR DENTRO DEL ARRAY DE RESPUESTA IGUAL A 1
                  $response->proceso = 1;
              }
          } else {
