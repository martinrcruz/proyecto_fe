...
class Club extends CI_Controller
{
    ...
    public function getClub()
    {
        if (true) {
            ...
            if ($query = $this->club_model->getClub()) {
                foreach ($query->result() as $res) {
                    $row = null;
                    $row = new stdClass();
                    $row->id_club = $res->id_club;
                    $row->nombre_club = $res->nombre;
    ...
    public function getDetalleCotizacionTabla()
    {
        if (true) {
            ...
            if (sizeof($response->errores) == 0) {
                if ($query = $this->club_model->getDetalleCotizacion($where)) {
                    foreach ($query->result() as $res) {
                        $row = null;
                        $row = new stdClass();
                        $row->id_club = $res->ID_DETALLE_COTIZACION;
    ...
    public function getDetalleCotizacionById()
    {
        if (true) {
            ...
            if (is_numeric($this->input->post('id_club'))) {
                $request->id = trim($this->security->xss_clean($this->input->post('id_club', true)));
            } else { //SI NO, ALMACENAMOS EL ERROR EN UN ARRAY PARA DEVOLVERLO COMO RESPUESTA.
                $response->errores[] = "Ocurrió un problema al obtener la solicitud";
            }

            $request->id ? $where = " AND dc.id_club=$request->id" : $where = '';
    ...
    public function insertDetalleCotizacion()
    {
        if (true) {
            ...
            if ($query = $this->club_model->insertDetalleCotizacion('club', $datos)) {
                $response->proceso = 1;
                $response->id = $query;
                $response->data = $datos;
            } else {
    ...
    public function updateDetalleCotizacion()
    {
        if (true) {
            ...
            if (sizeof($response->errores) == 0) {
                if ($query = $this->club_model->updateDetalleCotizacion('club', 'ID_DETALLE_COTIZACION', $datos, $request->id)) {
    ...
    public function deleteDetalleCotizacion()
    {
        if (true) {
            ...
            if ($this->input->post('id_club')) {
                $request->id = $this->security->xss_clean($this->input->post('id_club'));
            } else { //SI NO, ALMACENAMOS EL ERROR EN UN ARRAY PARA DEVOLVERLO COMO RESPUESTA.
                $response->errores[] = "Ocurrió un problema al obtener la solicitud";
            }

            $where = " AND id_club=$request->id";


            //SI ES QUE NO HAY ERRORES, PROCEDEMOS A HACER LA PETICION MEDIANTE UN LLAMADO A LA FUNCION DEL MODELO.
            if (sizeof($response->errores) == 0) {
                if ($this->club_model->updateDetalleCotizacion("club", "ID_DETALLE_COTIZACION", array('FECHA_BAJA' => $fecha, "ESTADO" => 0), $request->id)) {
                    //SI EL PROCESO ES EXITOSO, DEVOLVERA UN VALOR DENTRO DEL ARRAY DE RESPUESTA IGUAL A 1
                    $response->proceso = 1;
                }
            } else {
    ...
    public function deleteDetallesCotizacion(){
      if (true) {
          ...
          //SI ES QUE NO HAY ERRORES, PROCEDEMOS A HACER LA PETICION MEDIANTE UN LLAMADO A LA FUNCION DEL MODELO.
          if (sizeof($response->errores) == 0) {
              if ($query = $this->club_model->deleteDetalles($where, $fecha)) {
                  //SI EL PROCESO ES EXITOSO, DEVOLVERA UN VALOR DENTRO DEL ARRAY DE RESPUESTA IGUAL A 1
                  $response->proceso = 1;
              }
          } else {
